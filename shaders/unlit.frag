...
void main() {
    // ambient
    vec3 ambient = vec3(texture(diffuseSampler, fragTexCoord) * vec4(material.ambient, 0));
        
    // diffuse
    vec3 diffuse = vec3(texture(diffuseSampler, fragTexCoord) * vec4(material.diffuse, 0));
        
    // specular
    vec3 specular = vec3(texture(specularSampler, fragTexCoord) * vec4(material.specular, 0));
